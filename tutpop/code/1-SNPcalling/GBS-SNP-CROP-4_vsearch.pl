...
# Requirement 2: VSEARCH bioinformatics tool (Edgar, 2010)
##########################################################################################
...
# 4. Estimating the proportion of reads to be sampled, if necessary
	...
	if ($sum <= 0) {
		...
		."Please, see the GBS-SNP-CROP User Manual 'STAGE2: BUILD THE MOCK REFERENCE' for more detail.\n\n";
		goto exit;
	}

	# Defining the value of sampling proportion based on Usearch maximum memmory allowed
	my $prop_sampling;
...
# 5. Use USEARCH to cluster reads
	...
	if ($prop_sampling >= 1) { # Sub-sampling is not required.
		print "\nYour Mock Reference will be assembling using 100% of reads from the selected genotypes.\n";
		...
		system ( "./vsearch -sortbylength $UsearchIN --output $out5A" );
		...
		system ( "./vsearch -cluster_fast $out5A -id $id -threads $threads -consout $out5B -sizeout");
		...
		system ( "./vsearch -sortbysize $out5B --output $out5C" );
		...
		system ( "./vsearch -cluster_fast $out5C -id $id -threads $threads -consout $UsearchOUT" );

		print "All sub-steps for clustering population-level centroids were completed!\n";
		unlink $UsearchIN;

	# Appling random sampling
	} elsif ($prop_sampling < 1) {
		my $percent = $prop_sampling * 100;
		print "\nDue the memory consumption limit from 4 Gb imposed by Usearch (Zhang et al., 2014) and based on FASTA files size, the GBS-SNP-CROP will randomly sampling $percent% of reads before assembly the Mock Reference.\n\n";

		foreach my $file (@MR_taxa_files) {
			my $A_S_in = join(".","$file","AssembledStitched","fasta");
			my $sub = join (".","$file","AS","sub","fasta");

			print "Randonmly sampling reads from $A_S_in file ...\n";
...
# 6. Defining THE Mock reference, masked file and Delete N-containing centroids in order to create the Mock Reference
...
# 2. Estimating the proportion of reads to be sampled, if necessary
...
# 3. Use USEARCH to cluster reads 
	...
	# Appling random sampling
		...
		my $percent = $prop_sampling * 100;
		print "\nDue the memory consumption limit from 4 Gb imposed by Usearch (Zhang et al., 2014) and based on FASTA files size, the GBS-SNP-CROP will randomly sampling $percent% of R1 reads before assembly the Mock Reference.\n\n";

		foreach my $file (@MR_taxa_files) {
			my $in = join(".","$file","R1","fasta");
			my $sub = join (".","$file","R1","sub","fasta");

			print "Randonmly sampling reads from $in file ...\n";
...
# 4. Defining THE Mock reference, masked file and Delete N-containing centroids in order to create the Mock Reference
